...
| 10 | picker 三条 | 名字互不相同、都加得上去；**且每条缩略图的版式要与它对应的真卡对得上**（4×1 = 七列细块+胶囊块、4×3 = 左侧竖条+右侧三行、4×5 = 标题块+7×6 方格）—— 「三张各不相同」不算过：三张都塌成空卡也是「各不相同」 |
| 11 | 不可拉伸 | 长按卡片**没有缩放手柄** |
| 12 | 「6 行月 + 用户全部班次色」整屏截图并**逐格核对** 42 格是否都画出来了 | 探针只采过 6 与 42 色两点，7~41 色未知；半画满的月历是「卡片理直气壮地写错」那一类 |
| 13 | 4×1 的**占位态**不裁切（新装/升级后第一眼就是它） | 图标与 App 名都要完整、四边不切 |

> ℹ️ **一处已知无害的升级残留，别把它当谜案查**：旧版本（v0.8.7 及更早）排下的那枚刷新闹钟，
> 其目标组件 `ShiftWidgetProvider` 已随本轮删除，因此**无法被新版的 `WidgetRefreshScheduler.cancel()`
> 匹配**（`PendingIntent` 按「requestCode + 意图的 `filterEquals`」认身份，目标组件不同就不是同一个）。
> 它会在原定的那个时刻触发一次、落空 —— `WidgetRefreshReceiver` 收不到，卡片不受影响 ——
> 此后不再重排。**空转一次即止。**
