...
| 日期字号 | 18sp | 22sp |
| 班次名字号 | 15sp | 18sp |
| 农历 | `lunarShort` 一行 | `lunarFull` 最多两行，14sp |
| 行间距 | 8dp | 16dp |
| 徽章高 | 20dp | 24dp |
| 左侧色条 | 4×18dp | 4×24dp |

**2026-09-20 真机裁决（用户看过真机图之后）**：初稿的 18dp / 20sp / 17sp / 12dp / 22dp 档
在机上是「上下留了好大的空白」—— 内容约 200dp、4×3 可用约 265dp，约 65dp 余量被
`gravity="center_vertical"` 上下平分。**这份裁决推翻了初稿「余下约 68dp 由上下均匀留白
吃掉，不靠继续放大字号去填满」那条**：留白不是设计意图，是没排满。实际决定是
**字号与间距各提一档，把余量收进排版**（上表即提档后的值，`WidgetRenderer.renderTodayCard`
里色条与三个徽章的位图高同步 22→24dp —— 位图高与视图高必须相等，否则 `fitXY` 非等比拉伸）。
目标余量 **≤ 25dp**。**行高与结构没动**，改的只有字号、间距与徽章/色条这几处尺子上的
一档；「舒展」仍由真机图判定。

**2026-09-20 提档后真机复测**：内容实测约 **232dp**（改前约 202dp），4×3 外壳实测 266dp
（400dp 宽 / 480dpi 那台，密度 3.0，卡盒 739..1537px），**余量约 34dp** —— 比改前的约
65dp 收了一半，但**没到 ≤ 25dp**，差约 9dp。差额出在这一档自己身上：表里各项 +Δ 相加
只有约 +29dp（padding +8、日期 +2.8、农历两行 +2.8、班次 +1.4、三处行距 +12、时间间距
+2；徽章与色条 22→24dp 不改变行高，因为它们所在行的行高由文字撑），而「245dp」这个
目标数隐含的是 +43dp。**没有为此再自行加一档** —— 目标值是裁决给的，短期再猜一版不如
把实测数摆出来。要收掉这 9dp，最小改动是行距再 +2dp（16→18dp，三处）或日期再 +1sp。
...
**高度预算**：卡内可用约 450dp − 标题 18 − 周几 15 − 两个间距 8 ≈ 409dp。改法见下：
**行自己把余量吃掉** —— 42 个槽位 `android:layout_height="0dp"` + `android:layout_rowWeight="1"`，
`GridLayout` 自己 `android:layout_height="0dp"` + `android:layout_weight="1"` 吃掉标题/周几行
剩下的全部高度。于是 5 行月每行 ≈ 77dp、6 行月每行 ≈ 65dp —— **两种月份都铺满**。

**2026-09-20 真机裁决**：初稿是「网格高 `wrap_content` + 根 `gravity="center_vertical"`，
多余槽位 `GONE`，网格整体垂直居中」，5 行月的后果是卡下半部堆一坨空白（真机上约 113dp，
用户看到格子底下约 87dp 的空白）。**这份裁决推翻了初稿那条「不为凑满而拉高格子、
上下均匀留白」**：留白不是设计意图，是没铺满。现改为 `rowWeight` 按**可见**行分配余量
（`GONE` 的前导/尾随空格不参与布局，沿用 v0.8.6 的结论），每格的 59dp 内容在两种行高下
都由 `gravity="center"` 自然居中、更舒展。真机图上 5 行月与 6 行月**各核一张**。
...
## 8. picker 缩略图（`previewLayout` + `previewImage`）
...
| 路线 | 代价 |
|---|---|
| **A. 结构示意图**（初稿取它；⚠️ 2026-09-20 真机裁决：**在这台启动器上没生效** —— picker 里显示的仍是应用图标） | 每张卡出一张静态布局：只有形状与色块（网格的方格、本周条的一横排、今日卡的色条与色块），**无文字、无假数据**。好处：零双语问题、不撒谎、一眼能区分三张 |
| B. 真内容的静态布局 | 要写死示例文字（中文）→ 撞双语；写英文 → 中文用户看不懂。两条都要 `values-zh/en` 各一套布局 |
| C. `previewImage`（⚠️ 2026-09-20 与 A 一起采用；**不用 PNG 而用 `<vector>`**） | 初稿担心「一份管线活」。实际改用 `<vector>` 画：分辨率无关、无需 PNG 管线、无任何文字。代价只有「改版式时要一起改」 |

**2026-09-20 真机裁决**：用户的系统 picker 里，「倒班助手Pro」三条的缩略图**全是应用图标**
（HyperOS / Android 16），而竞品条目在同一台机器上能显示内容 —— 说明这台启动器**会**渲染
预览，只是没用上我们的 `previewLayout`（可能它只认 `previewImage`，也可能缓存了旧包
`initialLayout="@layout/widget_placeholder"` 的应用图标预览）。故 **A 与 C 两条一起上**：

- 三个 `previewLayout`（`widget_preview_strip` / `_today` / `_month`）**保留** —— 换一台
  启动器可能就认。
- 另补三张 `previewImage`：`widget_preview_strip_img` / `_today_img` / `_month_img`，
  用 `<vector>`，配色沿用 `widget_card_light` 的 `#F7FFFFFF` 与 `widget_preview_block` 的
  `#556E6E82`；形状按真卡比例（4×1 扁、4×3 方一点、4×5 高）。
- 三份 `res/xml/*_info.xml` 各加一行 `android:previewImage`。

**缩略图是否真被这台启动器用上，需用户手动点开 picker 复查**（这一步没法自动化验）。
...
| 月历 5 行 vs 6 行 | 多余槽位 `GONE`，可见行按 `rowWeight` 分配余量、网格铺满（§5.3） |
...
| 2026-09-20 | 真机反馈后修订三处（用户在机上看过成品图之后）：**① §5.2** 推翻「余下约 68dp 由上下均匀留白吃掉」，改为字号与间距提一档把余量收进排版（目标余量 ≤ 25dp，用户原话「上下留了好大的空白」）；**② §5.3** 推翻「网格整体垂直居中、上下均匀留白」，改为槽位 `rowWeight` 让行自己吃掉余量、网格铺满（5 行月每行 ≈ 77dp）；**③ §8** 路线 A（`previewLayout` 结构示意图）在这台启动器（HyperOS / Android 16）上没生效（picker 里显示应用图标），实际采用 **A + C** —— `previewLayout` 保留、另补三张 `<vector>` 的 `previewImage` |
