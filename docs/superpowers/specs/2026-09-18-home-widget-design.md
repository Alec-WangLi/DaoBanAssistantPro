...
所有「今天 / 白班 / 08:30 / 9月18日 / 周四」都是 Dart 用现成的 `L10n` 生成好的。Kotlin 侧**用户可见**的中文字符串一个都不出现（唯一的例外见 §8.4 的降级态，那里刻意不含任何句子）。双语只有 `l10n.dart` 一处要维护。

> 说「用户可见」是因为不变量 A 的本意是「别让原生攒出第二套 i18n」，不是「Kotlin 文件里不许出现汉字」—— 原生侧本来就有一堆 `AlarmLog.*` 的中文日志串（仓库既有惯例，与 i18n 无关）。写成「一个中文字面量都没有」是绝对化过头：后人打开文件看到满屏中文日志，会以为中文在这里是合法的，于是把**界面文案**也塞进来，那才是真毁掉这条不变量。
