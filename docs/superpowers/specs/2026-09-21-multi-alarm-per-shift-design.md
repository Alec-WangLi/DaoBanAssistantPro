...
- **00:00 上班的夜班 + 班中 03:00 补觉 → 当天 ✓（窗内）** —— 2026-09-21 用户点名问的就是这一条：
  同一个人两个闹钟，起床 23:00 归**前一天**、班中 03:00 归**当天**，各归各的，不需要用户额外指定。
  （旧规则会把 03:00 排到前一天，等于提前 21 小时响。）
- 上班或下班时间没填 → 窗口算不出来，**退回旧规则**（只有上班钟点时仍判「晚于上班钟点 → 前一天」）；
  上班时间也没填 → 判不了，**按当天**（沿用「不瞎挪一天」的口径）

### 4.3 与现行行为的差异面（必须写清，免得被当成回归）

只有一档不同：**闹钟钟点落在值班窗口内**的那些（白班的午休、零点班的班中补觉）。
那一档的旧行为（排到前一天）几乎必然是错的 —— 它把「班中间的闹钟」搬到了 20 多小时前。
其余钟点全部逐条相同，包括两个边界，都写进注释与测试：

- **恰好等于下班时刻**的钟点落在窗口之外（`end` 取开区间），走第二条分支，于是它落哪天
  **取决于它相对上班钟点的大小**：00:00–08:00 的 08:00 晚于上班钟点 → 前一天；
  20:00–08:00 的 08:00 早于上班钟点 → 当天。两种形状都与旧规则**逐字相同**。
- **下班之后**的钟点同理（00:00 班的 22:00 → 前一天；20:00 班的 09:00 → 当天），也逐字相同。

这两类钟点用户真想要的其实是「**次日**」（下班那一刻叫我），而这套模型只有「当天 / 前一天」
两档，表达不了 —— 见 §9「明确不做」。本轮刻意只保语义、不加档位：旧行为不会被改坏，
但它们也不会被修对。
...
| 文件 | 加什么 |
|---|---|
| `shift_rotation_test.dart` | 新规则的每一条边界：窗内 / 窗外早于上班 / 窗外晚于上班 / 正好等于上班 / 正好等于下班 / 下班之后 / 跨午夜班 / 24 小时班 / 上班时间没填 / 下班时间没填。**零点班那条照用户的例子写**：00:00–08:00 班配 23:00 与 03:00 两个闹钟 → 前者前一天、后者当天 |
