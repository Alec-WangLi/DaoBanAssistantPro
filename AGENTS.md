...
- **班次定义的 id 必须稳定（按天改班的地基）**：`saveSchedule` 从前是把该方案的班次行**删光重建**，于是每保存一次所有 `classId` 全变 —— 按天覆盖会在第一次保存后**静默指飞**。现已改成**增量**（带 id 的 UPDATE、没 id 的 INSERT、不在新列表里的 DELETE）。因此 `ShiftClass.id` 这条链上一个环节都不能漏：`ShiftClassRowX.toDomain()` 必须带出 id、编辑器的 `_editClass` 必须经 `copyWith` 把它传下去、`saveSchedule` 必须按 id 走增量而不是重建 —— 漏任何一环都不报错，只让那天的覆盖指向别的班次或指向空。删班次时引用它的覆盖行一并清掉（`saveSchedule` 收尾扫一遍悬空 `classId`）；`deleteSchedule` / `clearAll` 也要连带删覆盖。
