...
- **v0.7.1**（测试版 · 日历格子重做 + 自定义铃声 + 待办提醒修复）：三块独立改动。
  ...
  **重启重排（v0.7.5 起）**：班次 / 自定义 / 待办联动闹钟重启后不再丢 —— `AlarmScheduler.schedule` 排定时**同步落盘**一份清单（`AlarmStore`，存的是**排定参数**而不是「下次该响的时刻」：重复闹钟开机后要按 `nextDaily` / `nextWeekly` 重算，存时刻只会存出一条已经过期的记录），开机由 `BootReceiver` 读清单重排：重复的算下一次、一次性且还没到点的按原时刻排回、**关机期间错过的一次性闹钟直接丢掉**（半夜的闹钟在早上开机时突然响，比不响更糟）。**每条取消路径都必须同步删**（`AlarmScheduler.cancel`、`cancelAllNativeAlarms` 里的 `AlarmStore.clear`），漏一处就留下一条「幽灵闹钟」—— 界面上看不出来，直到某天重启后它自己响了。**已知边界**：① 设备设了锁屏密码时 `BOOT_COMPLETED` 要等**首次解锁之后**才送到非 Direct Boot 应用（真机实测：重启后解锁前闹钟数 0，解锁瞬间清单 30 条全部排回，时间是准的）；要覆盖「半夜重启、一直没解锁」那段得让存储与接收器都走 Direct Boot，改动更大，先不做。② **待办提醒（`scheduleQuiet` 那条链路）没有纳入清单**，重启后依旧要等下次打开 App 才重排。③ 小米机型还要用户开「自启动」，否则 MIUI 可能压根不把开机广播发给 App。
