# 倒班助手Pro · 产品与技术规格（现状规格 · v0.8.6）

> 本文档是**唯一权威规格**，反映当前实现现状（v0.8.6）。早期「MVP 确认稿」的功能已随 0.2~0.6 各版演进并入正文，不再单列历史章节。
...
### 联动班次闹钟
10. 每个班次独立设置响铃时间与开关；`AlarmService.reschedule` 按排班自动排定**未来 60 天**并自动续排。**按天改班自动流进闹钟排定**：改成休班那天不排闹钟，改成别的班次就按那个班自己的响铃时间排。这与「按天关闹钟」（`ShiftAlarmOverrides`）**正交**，两条规则各管各的 —— 那天被单独关过闹钟的，改完班之后仍然关着。
...
### 桌面小组件
22. **桌面小组件（五档尺寸自适应）**：**五档由宿主给的高度决定，宽度只当网格的闸门**（`WidgetTier.pick`，阈值取真机实测七个可达高度 —— 63 / 158 / 253 / 348 / 443 / 538 / 633dp —— 的相邻中点）—— 高 <110dp → `LIST_COMPACT` 两行紧凑列表（今天 + 明天，行高 22dp）；<205dp → `LIST_3` 三行列表；<300dp → `LIST_5` 五行列表；高 ≥300dp **且** 宽 ≥300dp（4 列）→ `GRID_WEEK` 一周网格（8 格，末格空）；高 ≥490dp 且宽 ≥300dp → `GRID_FORTNIGHT` 两周网格（16 格，末两格空）。两档网格下方各附一张**今日卡片**（照搬 App 底栏信息卡：农历、其他班组当天班次、当天待办数；徽章文案由 Dart 侧快照给）。**高够但宽不足（窄长条）降级为 `LIST_5`** —— 本轮网格只有 4 列一种，排不下就退回最长的那档列表。旧的三档把三个版式摊到七个高度上，行高会被拉伸、格子又大又空；这一轮按高度细分，每档都有一版对得上的版式。**走原生 `RemoteViews` 渲染，不是 Flutter** —— Dart 侧 `features/widget/widget_snapshot.dart` 产出「已本地化的快照」（班次名 / 简称 / 时间串 / 颜色 / 深浅模式 / 语言 / 刷新边界）写进 `SharedPreferences`（`shift_widget`/`snapshot`），原生 `ShiftWidgetProvider` 只排版，**用户可见文案全部来自 Dart 侧快照**（双语只有 `l10n.dart` 一处；Kotlin 侧只有 `AlarmLog` 的日志串是中文，界面文案不在其中）。**跨天自动翻页 + 班次边界刷新**（在 App 里改排班 / 主题 / 语言后启动一次即同步；快照带 14 天，跨天由原生按 epoch day 对表右移、不重算，耗尽退化成占位态）；**今日卡片自带它的 `day`，跨天后若它不等于今天就换成降级态**（只显示 `days[todayIndex]` 的日期 / 班次 / 时间，农历与其他班组留空），不把旧数据当成今天显示；点整卡开 App、点网格某格跳到那天的日历。**不动 Drift schema**（快照走 SharedPreferences）。**小米 / HyperOS 上入口特殊**：标准 Android 小组件不进原生那一栏，藏在「支持小部件的应用」→「安卓小部件」二级分类里（长按桌面空白 → 添加小部件），README 与更新日志都写明了怎么找。
