...
2. **月视图**：每格的班次是个**带底色的胶囊**（班次色 14% 淡染底 + 45% 描边 + 按底色算可读的文字色），三行**全部居中**（日期 / 班次 / 农历）；格内文字按格子高度等比缩放（1.0–1.25×，格子长高字跟着长），胶囊里的简称再套一层 `FittedBox` 兜底 —— 两个字甚至三个字的简称都不会被截成省略号，系统字号放大也照缩。选中那天的胶囊变**实心**（滑到哪格一眼可见）；**无班次的方案**（跟随法定节假日）只是少画中间那一行；格子太窄（小窗）时胶囊退化为纯色文字。上/下月翻页（**方向性动画**：往前翻往左滑、往后翻往右滑，网格与年月胶囊同步，`AppTokens.durMed`）、一键回今天、**年月跳转选择器**（年份滚轮 + 4×3 月份网格）。**换月只有这两条路径**（网格上没有横向手势，横向拖动归底栏导航），所以这个位移不与任何手势冲突。

    **底栏信息卡的高度是「算」出来的，且只在换月时才可能变一次**。竖屏是 `Column[顶栏, Expanded(网格), 信息卡]`，格子高度按**剩余空间**均分 —— 卡片随当天内容长高一像素，六个格子就集体矮一像素、点下一天再弹回来。所以高度按**月**定：`info_card_metrics.dart` 用 `TextPainter` 按卡片里同一份样式与约束把**本月每一天**排一遍，**逐日取大**（不是把各项的月内最大值相加 —— 那会把「A 天有法定节假日徽章」和「B 天色块折三行」叠成一天的高度，而一个月里没有哪天真需要那么高）。代价只有一个：换月时高度可能变一次。**卡片那截富余**（最满的一天正好占满：400dp 宽、六班组、有节假日的月份实测普通日子空 32~51dp）由一行「**本月统计**」填上（`本月 早12 · 午8 · 夜8 · 休6`，班次用格子里那套 `shortLabel`）—— **只在当天富余装得下时才画**（`infoCardTallyFits`），绝不为此改卡片高度：于是网格一格都不抖，装不下的那天（节假日那种最满的日子）自动不画。右栏 / 横屏那张卡高度跟着内容走，它就常显。
...
### 联动班次闹钟
10. **每个班次最多 6 个联动闹钟**（各自的钟点 + 可选名字，如「起床」「午休」），加一个总开关（关掉**不清空**已配的钟点）；`AlarmService.reschedule` 按排班自动排定**未来 60 天**并自动续排。响铃标题取「班次名 · 闹钟名」（没起名就是「白班提醒」）。**按天改班自动流进闹钟排定**：改成休班那天不排闹钟，改成别的班次就按那个班自己的闹钟排。这与「按天关闹钟」（`ShiftAlarmOverrides`）**正交**，两条规则各管各的 —— 那天被单独关过闹钟的，改完班之后仍然关着（关的是当天这个班次的**全部**闹钟，不是逐个）。**落在哪天的规则**（`alarmFallsOnPreviousDay` / `shiftAlarmFireAt`）：钟点落在**值班窗口** `[start, end)` 内 → 班次**当天**（白班的午休 12:30、零点班班中的 03:00 是这一档）；否则**晚于上班钟点** → **前一天**同一钟点（00:00 上班、23:00 响铃的夜班排的是前一天晚上 23:00 —— 排在班次当天那一刻这个班已经结束 15 小时）；否则当天。班次编辑里落前一天的那条写成「前一天 23:00」并补一行说明，闹钟列表行与日历信息卡同样标「前一天」（多条时**每条标各自的**）；上班时间没填时无从判断、仍按当天算。原生 id 与 `offset` 始终按**班次那一天**算，不跟着前移的触发时刻走；id = `序号 × 60 + 天数偏移`，**上限 6 就是这么来的**（Kotlin 侧只扫 0..400）。**不做**「次日」（下班之后那一刻）—— 这套两档模型表达不了。
11. **未来 30 天预览列表**：每条可单独开关（写入按天覆盖表 `ShiftAlarmOverrides`，主键 `day` = 自 epoch 天数，由 `dayNumber()` 计算），响过后自动隐藏；`Timer.periodic(1min)` + 回前台重建刷新。
12. **自定义闹钟**：一次性 / 每天 / 每周（多选星期，位掩码）；重复型由原生侧同一 id 续排；新建默认时间即此刻，**重复方式默认「一次性」**（v0.9.6 起，原先默认「每天」），一次性那行的日期由 `nextOnceDate` 取「该钟点的下一次出现」——写「今天」的话那一刻已经过去了，这种闹钟既排不出去又会被 `deleteExpiredOnceAlarms` 收走（判据与边界见 `alarm_service.dart` / `once_alarm_date_test.dart`）。
...
- **背景光晕（`core/theme/animated_background.dart`）**：`FlowingBackground` 在两处铺底 —— **响铃界面**（强度 1.0，它本来就有的）与 **日历页**（0.65）。它**自己不画底色**，只在 `Scaffold` 那块平坦底色上叠三团缓慢漂移的光斑，给磨砂玻璃一层可以透的**弱结构**（在此之前日历页是一块平色，满页磨砂没在磨东西、只靠高光与描边撑着）。**驱动是低频定时器（一步借 `AppTokens.durSlow`），不是 `AnimationController..repeat()`** —— 26 秒漂 46dp（1.8dp/秒），逐帧画每帧只动 0.03dp、没人看得出来，却会让整页永久不空闲、压在它上面的每一层 `BackdropFilter` 每帧重算；而且 `repeat()` 会让 `pumpAndSettle` 永远等不到停（日历页的 widget 测试会整片红）。**别改回逐帧。** 日历页另传 `freezeWhenBlurDisabled: true`：「高级材质」一关就停下 —— 那个开关的语义是「这台机器不做贵的合成」，一边关模糊一边继续推背景等于把省下的又花回去（响铃界面不传，行为与从前一致）。**观感在深浅两档差别很大**：日历页那 42 个格子的卡底是 92% 不透明白（浅色）/ 6% 白（深色），所以浅色下光只能从格子缝、网格四周留白与信息卡的磨砂里透出来（很轻），深色下几乎全透、一眼可见。
