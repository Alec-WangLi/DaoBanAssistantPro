# 倒班助手Pro · 产品与技术规格（现状规格 · v0.8.4）

> 本文档是**唯一权威规格**，反映当前实现现状（v0.8.4）。早期「MVP 确认稿」的功能已随 0.2~0.6 各版演进并入正文，不再单列历史章节。
...
### 桌面小组件
22. **桌面小组件（三档尺寸自适应）**：小 / 中 / 大三档由宿主给的 dp 尺寸决定（`WidgetTier.pick` —— 宽 ≥300 且高 ≥260 → 大卡「一周一览」4×4 网格；宽 ≥220 且高 ≥130 → 中卡今天 + 未来三天；否则小卡今天与明天），不是固定的 1×1 / 2×2 / 4×4 标签。**走原生 `RemoteViews` 渲染，不是 Flutter** —— Dart 侧 `features/widget/widget_snapshot.dart` 产出「已本地化的快照」（班次名 / 简称 / 时间串 / 颜色 / 深浅模式 / 语言 / 刷新边界）写进 `SharedPreferences`（`shift_widget`/`snapshot`），原生 `ShiftWidgetProvider` 只排版，**用户可见文案全部来自 Dart 侧快照**（双语只有 `l10n.dart` 一处；Kotlin 侧只有 `AlarmLog` 的日志串是中文，用户可见的中文一个字面量都不出现）。**跨天自动翻页 + 班次边界刷新**（在 App 里改排班 / 主题 / 语言后启动一次即同步；快照带 14 天，跨天由原生按 epoch day 对表右移、不重算，耗尽退化成占位态）；点整卡开 App、点大卡某格跳到那天的日历。**不动 Drift schema**（快照走 SharedPreferences）。**小米 / HyperOS 上入口特殊**：标准 Android 小组件不进原生那一栏，藏在「支持小部件的应用」→「安卓小部件」二级分类里（长按桌面空白 → 添加小部件），README 与更新日志都写明了怎么找。
